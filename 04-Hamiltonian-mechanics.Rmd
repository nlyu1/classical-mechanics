...
:::{.definition name="canonical transform"}
A map $\xi\mapsto \varphi(\xi)$ is a canonical transform 
if it is a _symplectomorphism_, i.e. it preserves the canonical 
form $\omega = \sum_{j=1}^n dq_j\wedge dp_j$ under the pullback map 
\[
    \varphi^*\, \omega = \omega 
\]
In components, this is equivalent to $J\Gamma J^T = \Gamma$, where 
$J$ is the Jacobian of the transformation $\varphi$. 
:::

:::{.theorem name="Liouville's theorem"}
A canonical transform is a probability-preserving transformation 
(i.e. preserves phase space volume). 
Equivalently, the Jacobian $J$ satisfies $\det J=1$ 
in the $\xi$ basis. 
:::
_Proof:_ The $n$-th exterior product $\omega^n$ is the volume form. 

We will state, instead of prove, the following result; the 
most concise proof constitutes computing the Lie derivative of 
$\omega$ under $D_A$ using Cartan's formula. 
...
\]
In other words, every transform of phase space generated by 
Hamiltonian flow is a canonical transform. 
:::